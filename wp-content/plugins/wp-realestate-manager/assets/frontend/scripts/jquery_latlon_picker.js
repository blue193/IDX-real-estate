...
// ^^^
    ...
    jQuery.fn.gMapsLatLonPicker = (function (readius) {
        ...
        var performSearch = function (string, silent) {
            if (string == "") {
                var postiting_lat = jQuery('#wp_rem_post_loc_latitude').val();
                var postiting_lon = jQuery('#wp_rem_post_loc_longitude').val();
            ...
            _self.vars.geocoder.geocode(
                    ...
                    function (results, status) {
                        if (status == google.maps.GeocoderStatus.OK) {
                            ...
                            wp_rem_hide_button_loader('.search-location-map');
                        } else {
                            if (!silent) {
        ...
        var publicfunc = {
            ...
            init: function (object) {
                ...
                if (jQuery('#btn-next-advance-options').length > 0) {
                    jQuery('#btn-next-advance-options').click( function(){
                        var mapResizeTimes = 0;
                        setTimeout(function () {
                            if (mapResizeTimes === 0) {
                                google.maps.event.trigger(_self.vars.map, "resize");
                                _self.vars.map.setCenter(new google.maps.LatLng(_self.params.defLat, _self.params.defLng));
                                mapResizeTimes++;
                            }
                        }, 500);
                    });
                ...
                jQuery(_self.vars.cssID + ".gllpSearchButton").bind("click", function () {
                    ...
                    wp_rem_show_loader('.search-location-map', '', 'button_loader', thisObj);
                    wp_rem_gl_search_map(jQuery('#post_loc_address').val());
